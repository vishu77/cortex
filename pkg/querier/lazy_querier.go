...
func (l lazyQuerier) Select(params *storage.SelectParams, matchers ...*labels.Matcher) (storage.SeriesSet, error, storage.Warnings) {
	future := make(chan storage.SeriesSet)
	go func() {
		set, err, _ := l.next.Select(params, matchers...)
		if err != nil {
